...
Score each report from 0 to 4 for every dimension. Use a fresh context for the baseline and enhanced runs.
...
| Link health | Links missing/broken | Links exist but are partly unchecked | Links are complete, accessible, and title-relevant |
| Usefulness | Generic or impractical | Some useful context | Concise, decision-relevant, and readable |

## Acceptance threshold

The enhanced report passes when no critical dimension is below 3, no high-impact claim is unsupported, and the report clearly states limitations. Do not use an average score to hide a traceability or citation-correctness failure.
